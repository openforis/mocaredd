# mocaredd - package creation steps

``` r
#library(mocaredd)
```

This page shows the different steps to create the package and the main
function which is a shiny app, as well as how the app is loaded to
shinyapp.io.
...
2.  Create a R project from Github (See <https://happygitwithr.com/> on
    linking Github repos to Rstudio).
3.  Create a package structure inside he project:
    - `usethis::create_package("PATH/mocaredd")` with PATH the path to
      the project folder set during the project creation.
