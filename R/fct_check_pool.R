...
#'
#' @param .c_sub Subset of the Carbon Stock input table (c_stock) for a unique land use
...
#'
#' fct_check_pool(.c_sub = c_sub, .c_unit = "C", .c_fraction = NA)
#'
#' @export
fct_check_pool <- function(.c_sub, .c_unit, .c_fraction){

  ## !! FOR TESTING ONLY
  # .c_sub    <- .cs |> filter(c_lu_id == "dg_ev_wet_closed")
  # .c_unit <- "C"
  # .c_fraction <- NA
  ## !!

  ## Get pools for the input data subset
  c_pool  <- .c_sub |> pull(c_element) |> sort()
  lu_id   <- .c_sub |> pull(c_lu_id) |> unique()
  ...
  # c_status <- .c_sub$c_status |> unique()
